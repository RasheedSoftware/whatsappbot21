...
const loadBlockedcontact= () => {
    if (fs.existsSync(pathContact)) {
        ...
        const parsedData = JSON.parse(data);
        blockedcontact = Array.isArray(parsedData.blockedcontact) ? parsedData.blockedcontact : []; // تأكد من أنه مصفوفة
    } else {
...
// Add a help request
const addHelpRequest1 = ( request) => {
    if (helpRequests && !helpRequests.includes(request)) {
        helpRequests.push(request);
        saveHelpRequests();
        console.log(`Added help request: ${request} to category: `);
    } else {
        console.log(`Request "${request}" is already in the category or category does not exist.`);
    }
};
